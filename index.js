...
client.on('message', msg => {
    if((msg.channel.id === introChannelId || msg.channel.id === botChannelId) && !msg.author.bot) {
        const user = parseMessage(msg.content);
        console.log(user);

        if(user) {
            assignRoles(msg, user);
            msg.reply('welcome to the server! :)');
        }
        else {
            msg.reply('I was unable to parse your introduction, please try again! :(');
        }
    }
});
